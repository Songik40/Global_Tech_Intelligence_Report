...
로봇·자율주행은 **기술 데모 → 실제 배치 → 인증·신뢰성·경제성 검증 → 지역 생산·상업 운행 → 인프라 규모 통합** 단계로 이동하고 있다.
...
중국 휴머노이드 업체, 글로벌 산업용 로봇 기업, 자율주행 서비스 사업자, 자동차 제조사, foundation robotics 연구그룹, 지역 제조 파트너, 저가 로봇 개발 플랫폼, 물류·광업·항만 운영자.
...
생산능력과 투자·기업 수는 빠르게 증가하지만, 반복 가능하고 수익성 있는 실제 작업 데이터는 상대적으로 부족하다. 공장 오픈·사전주문·`planned capacity`·목표 차량 수는 상용화 성과와 다르며 실제 인도량, 유료 이용량, 가동률을 별도로 봐야 한다. 국가 규모 프로젝트도 실제 상업 운행 데이터가 나오기 전에는 목표와 성과를 구분해야 한다.
...
> Physical AI와 자율주행의 승자는 가장 인상적인 데모보다 **신뢰 가능한 실제 데이터, 규제 통과, 지역별 적용 적합성, 대량생산·서비스 능력, 인프라 통합, 경제성**을 먼저 연결하는 쪽일 가능성이 높다.
